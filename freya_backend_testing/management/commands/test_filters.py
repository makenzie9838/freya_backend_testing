...
from freya_backend_testing.database_utils import create_test_data, filter_products_by_ingredient, filter_out_products_that_have_ingredient
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        self.stdout.write(self.style.SUCCESS(
            f'Products with ingredient "{ingredient}":'))
        for product in filtered_products:
            self.stdout.write(f'{product.name} - {product.ingredients}')

        # Test filtering out products by ingredient
        filtered_out_products = filter_out_products_that_have_ingredient(
            ingredient)
        self.stdout.write(self.style.SUCCESS(
            f'Products without ingredient "{ingredient}":'))
